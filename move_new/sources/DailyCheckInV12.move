module mines::daily_check_in_v12 {
    use std::signer;
    use aptos_framework::timestamp;
    use aptos_framework::event::{Self, EventHandle};
    use aptos_framework::account;
    use mines::badges_v12;
    ...
    public entry fun check_in(user: &signer) acquires CheckInState {
        ...
        if (state.current_streak == 7) {
            badges_v12::mint_badge(user, 1);
        } else if (state.current_streak == 14) {
            badges_v12::mint_badge(user, 2);
        } else if (state.current_streak == 30) {
            badges_v12::mint_badge(user, 3);
        } else if (state.current_streak == 90) {
            badges_v12::mint_badge(user, 4);
        };
